var Menu = pc.createScript('menu'); // Javascript Processor to create reference to script

Menu.prototype.initialize = function() { // Function to run when script is first called
    var css = [
            '.vel {',
                    'color: orange;',
                    'position: absolute;',
...
Menu.prototype.drawVel = function (velx, vely, velz) { // Function to redraw velocity display to be called in other scripts
    this.vel.innerHTML = pc.string.format("<div class='vel'><p> velocity x {0}</p><p> velocity y {1}</p><p> velocity z {2}</p></div>", velx, vely, velz);
};

Menu.prototype.destroyVel = function () { // Function to remove velocity display when changing scenes to be called in other scripts

var Particles = pc.createScript('particles2d2box'); // Javascript Processor to create reference to script
...
Particles.prototype.initialize = function() { // Function which runs when script is first called
    ...
    for (var i = 0; i < this.count; i++) { // For loop to generate particles
        ...
        newparticle.rigidbody.teleport(Math.random() * 8 + (- 8 * halfcheck), 0.1, Math.random() * 8 - 4); // Teleports particle to random initial location
        ...
        var initvel = Math.sqrt(this.initTotalEnergybox[halfcheck] * 2 / (2 * this.count)); // Calculates initial velocity for each particle

var Particles = pc.createScript('particles3d'); // Javascript Processor to create reference to script
...
Particles.prototype.initialize = function() { // Function which runs when script is first called
    ...
    for (var i = 0; i < this.count; i++) { // For loop to generate particles
        ...
        newparticle.rigidbody.teleport(Math.random() * 8 - 4, Math.random() * 8 - 4, Math.random() * 8 - 4); // Teleports particle to random initial location
        var linearfactor = new pc.Vec3(1.00, 1.00, 1.00);
        ...
        var initvel = Math.sqrt(this.initTotalEnergy / (3 * this.count)); // Calculates initial velocity for each particle
...
Particles.prototype.update = function(dt) { // Function which runs every time step
    if (this.app.timeScale !== 0) {
        for (var i = 0; i < this.Particles.length; i++) { // For loop to run for every particle
            ...
            var oldy = this.ParticleYVelocities[i]; // Sets oldy variable to save previous y velocity
            ...
            if (pos.x < -4) { // Negative x boundary condition
                particle.rigidbody.teleport(3.9, particle.position.y, particle.position.z);
            }
            ...
            if (pos.z < -4) { // Negative z boundary condition
                particle.rigidbody.teleport(particle.position.x, particle.position.y, 3.9);
            }
            if (pos.z > 4) { // Positive z boundary condition
                particle.rigidbody.teleport(particle.position.x, particle.position.y, -3.9);
            }
            if (particle.rigidbody.linearVelocity.x === 0 && particle.rigidbody.linearVelocity.y === 0 && particle.rigidbody.linearVelocity.z === 0) { // If loop to stop particles stopping completely
                var velocity = new pc.Vec3(oldx , oldy, oldz);
